...
async function fetchDetails() {
  ...
  loading.value = true;
  try {
    const data = await $fetch<Proposition>(`/api/propositions/${props.proposition.id}`, {
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    toast.add({ title: "Erreur", description: error?.message || "Mise à jour impossible", color: "error" });
  }
}
...
async function duplicateProposition() {
  ...
  try {
    await $fetch("/api/propositions", {
      ...
      headers: getAuthHeaders(),
      body: {
        commune_id: props.proposition.commune_id,
        name: `${state.name?.trim() || props.proposition.name} (copie)`,
    ...
    toast.add({ title: "Erreur", description: error?.message || "Duplication impossible", color: "error" });
  }
}
  ...
  <UModal v-model:open="open" title="Modifier la proposition" :ui="{ content: 'sm:max-w-5xl w-[95vw] h-[90vh]' }">
    <template #body>
      <div class="space-y-4">
        ...
        <div class="space-y-3">
          <h4 class="font-medium text-highlighted">Commentaires ({{ comments.length }})</h4>
          <div v-if="loading" class="text-sm text-muted">Chargement...</div>
          <div v-else-if="comments.length === 0" class="text-sm text-muted italic">Aucun commentaire</div>
          ...
          <div class="pt-2">
            ...
            <div class="flex gap-2 items-end">
              <UTextarea v-model="newComment" :rows="3" class="w-full" placeholder="Commenter en tant que mairie..." />
              <UButton
                label="Commenter"
                icon="i-lucide-send"
      ...
      <div class="w-full flex justify-between gap-2">
        ...
        <div class="flex gap-2">
          <UButton
            ...
            @click="toggleArchive"
          />
          <UButton label="Dupliquer" color="neutral" variant="subtle" icon="i-lucide-copy-plus" @click="duplicateProposition" />
          <UButton label="Annuler" color="neutral" variant="subtle" @click="open = false" />
          <UButton label="Enregistrer" color="primary" variant="solid" type="submit" :form="formId" />
        </div>
      </div>
    </template>
  </UModal>
