...
function openModal() {
  if (props.info) {
    ...
    state.content = props.info.content
    state.category = props.info.category || undefined
    state.image_url = props.info.image_url || undefined
    open.value = true
  }
}
...
<template>
  <UModal v-model:open="open" title="Modifier l'information" description="Modifier une information municipale">
    <template #body>
      <UForm
        :schema="schema"
        :state="state"
