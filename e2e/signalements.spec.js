import { test } from './fixtures/test-fixtures'

test.describe('Signalements smoke', () => {
  test.beforeEach(({ backofficeCredentials }) => {
    const { userEmail, userPassword } = backofficeCredentials
    test.skip(!userEmail || !userPassword, 'Missing BACKOFFICE_E2E credentials in .env.local')
  })

  test('user can access signalements page', async ({
    signalementsPage,
    backofficeCredentials,
    backofficeSession
  }) => {
    await backofficeSession.loginAsCommuneUser()
    await signalementsPage.goto()
    await signalementsPage.expectLoaded()
    await signalementsPage.expectCommuneContextVisible(backofficeCredentials.communeName)
  })
})
