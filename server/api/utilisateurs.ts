...
export default eventHandler(async (event) => {
  ...
  try {
    ...
    const { data: utilisateursData, error: utilisateursError } = await supabase
      .rpc('get_utilisateurs_with_last_sign_in')
    ...
    console.log('Found associations:', associationsData?.length || 0, 'Error:', associationsError?.message)
    ...
    if (communeIds.length > 0) {
      ...
      console.log('Found communes:', communesData.length, 'Error:', communesError?.message)
    }
