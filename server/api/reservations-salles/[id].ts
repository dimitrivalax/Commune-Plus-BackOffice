...
export default eventHandler(async (event) => {
  ...
  try {
    if (method === "PUT") {
      ...
      // Envoyer une notification si le statut change vers confirmé ou refusé
      if (
        body.status &&
        ...
        const { sendReservationNotification } =
          await import("../../utils/send-reservation-notification");
        ...
        // On envoie la notification de manière asynchrone pour ne pas bloquer la réponse API
        sendReservationNotification({
          supabase,
          reservationId: String(data.id),
          userEmail: data.email,
          userName: data.name,
          salleName: salleData.nom,
          date: displayDate,
          startTime: startTime,
          endTime: endTime,
          status: data.status,
        }).catch((err) => {
          console.error("Error sending reservation notification:", err);
        });
      }
