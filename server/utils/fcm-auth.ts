...
async function loadServiceAccount(): Promise<ServiceAccount> {
  ...
  if (serviceAccountJson) {
    try {
      ...
      throw new Error(`Failed to parse FCM_SERVICE_ACCOUNT_JSON: ${error.message}`)
    }
    ...
    try {
      ...
      throw new Error(`Failed to load FCM service account from ${serviceAccountPath}: ${error.message}. Make sure the file exists and contains a valid service account JSON with private_key field.`)
    }
  } else {
    throw new Error('FCM_SERVICE_ACCOUNT_JSON or FCM_SERVICE_ACCOUNT_PATH must be set. Use the service account JSON file (not google-services.json from mobile app).')
  }
  ...
  if (!serviceAccount.client_email) {
    throw new Error(`Service account JSON is missing client_email field. Available fields: ${availableKeys.join(', ')}`)
  }
  if (!serviceAccount.project_id) {
    throw new Error(`Service account JSON is missing project_id field. Available fields: ${availableKeys.join(', ')}`)
  }
  if (!serviceAccount.token_uri) {
    // Le token_uri peut être dérivé si absent
    serviceAccount.token_uri = serviceAccount.token_uri || 'https://oauth2.googleapis.com/token'
  }
...
async function createJWT(serviceAccount: ServiceAccount): Promise<string> {
  ...
  try {
    ...
    throw new Error(`Failed to create private key object: ${error.message}. Make sure the private_key in your service account JSON is valid.`)
  }
  ...
  try {
    ...
    if (!signature) {
      throw new Error('Failed to generate signature: sign() returned empty result')
    }
