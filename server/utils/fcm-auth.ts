...
async function loadServiceAccount(): Promise<ServiceAccount> {
  ...
  if (serviceAccountJson) {
    try {
      ...
      throw new Error(
        `Failed to parse FCM_SERVICE_ACCOUNT_JSON: ${message}`
      )
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to load FCM service account from ${serviceAccountPath}: ${message}. Make sure the file exists and contains a valid service account JSON with private_key field.`
      )
    }
  } else {
    throw new Error(
...
async function createJWT(serviceAccount: ServiceAccount): Promise<string> {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unknown key parsing error'
    throw new Error(
      `Failed to create private key object: ${message}. Make sure the private_key in your service account JSON is valid.`
    )
  }
