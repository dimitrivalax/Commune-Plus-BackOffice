...
export async function sendSignalementNotification(
  ...
  const { supabase, signalementId, userId, title, body, type, newStatus } =
    options;
  ...
  console.log(
    "Sending signalement notification for user :::::::::::::::::::::::: ",
    userId,
  );
  try {
    ...
    console.log("FCM configured:", accessToken, projectId);
    ...
    for (const token of allTokens) {
      try {
        ...
        if (!response.ok) {
          ...
          console.error("FCM FULL ERROR:", JSON.stringify(errorData, null, 2));
